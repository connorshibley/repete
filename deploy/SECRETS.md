...
| `FRED_API_KEY` | no | Macro research only. **Nothing in `src/` reads it** — it exists solely for `scripts/probe_alfred_vintages.py`, which decides whether ALFRED vintages may be BACKTESTED at all (a missing vintage axis, values identical across vintages, or a first-vintage date equal to the period each make a gate inflated and indistinguishable from a real edge). Absent = the probe refuses to run. Cannot affect trading. |
| `SEC_EDGAR_USER_AGENT` | no | **Not a credential** — contact metadata. EDGAR needs no key but requires a declared `User-Agent` (`"Name email@example.com"`) and throttles anonymous traffic. Used solely by `scripts/probe_edgar_pit.py`. Deliberately absent from `scripts/check_secret_exposure.py`, which answers "does this need rotating"; an email address cannot be rotated. Cannot affect trading. |
